...
**LLM：Codex Haiku**
`Codex-haiku-4-5-20251001`，用於 RAG 回答生成。回答語言固定為繁體中文，並要求引用格式 `[數字]`。
...
**資料庫遷移：手動 ALTER TABLE**
無 Alembic。新欄位透過 `database.py` 的 `_migrate_add_content_hash()` 在每次啟動時手動執行 `PRAGMA table_info` 檢查並補 `ALTER TABLE`。
...
- 測試資料庫用獨立 SQLite in-memory，向量庫用獨立 chroma client（`EphemeralClient`）
- 每個測試後清理狀態，不依賴執行順序
